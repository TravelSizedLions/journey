...
public static class Rand  {
	...
	/// </param>
	public static float Normal(float min, float max, ConfidenceLevel_e confidence_level_cutoff /*, float confidence_level_cutoff*/) {
		...
		do {
			random_normal_num = Normal(mean, sigma);
	...
	public static float Normal(float mean, float std_dev) {
		...
		float random_normal_num = Normal();
	...
	/// </summary>
	/// <returns>
	/// A random number in range [-inf, inf] from the standard normal distribution (mean == 0, stand deviation == 1).
	/// </returns>
	...
	public static float Slope(float min, float max, float skew, Direction_e direction) {
		return  min + Slope(skew, direction) * (max-min);
	}
	...
	/// <param name="skew">The difference in height between max and min of curve.</param>
	public static float Slope(float skew, Direction_e direction) {
	...
	public static float Linear(float min, float max, float slope) {

		float val = Linear(slope);
	...
	public static float Linear(float slope) {

		float abs_value = SlopedLinear(Mathf.Abs(slope));
		if (slope < 0) {
			return 1 - abs_value;
		}
		else {
	...
	// Returns random in range [0,1] with linear distribution of given slope.
	private static float SlopedLinear(float slope) {
	...
	public static float Exponential(float min, float max, float exponent, Direction_e direction) {
		return min + Exponential(exponent, direction) * (max-min);
	}
	...
	/// </param>
	/// <param name="direction">The direction for the curve (right/left).</param>
	public static float Exponential(float exponent, Direction_e direction) {
